...
class CompareBar {
    ...
    addbutton(item){
        if (document.getElementById("numOfEl").innerText == "3") {
            ...
            compareB.innerHTML = (`<button id=b${item.symbol} onclick='remove(this)'>${item.symbol} &nbsp; &#215;</button>`)
...
class List {
    ...
    load(item) {
        // console.log(item.profile.changesPercentage)
        // console.log(item.profile.changes)
        // let percents = item.profile.changesPercentage.slice(1,-2);
